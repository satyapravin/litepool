...
"""LitePool meta class for gymnasium.Env API."""
...
from .litepool import LitePoolMixin
from .utils import check_key_duplication


class GymnasiumLitePoolMixin(ABC):
...
class GymnasiumLitePoolMeta(ABCMeta, gymnasium.Env.__class__):
  """Additional wrapper for LitePool gymnasium.Env API."""

  def __new__(cls: Any, name: str, parents: Tuple, attrs: Dict) -> Any:
    """Check internal config and initialize data format convertion."""
    base = parents[0]
    parents = (base, GymnasiumLitePoolMixin, LitePoolMixin, gymnasium.Env)
