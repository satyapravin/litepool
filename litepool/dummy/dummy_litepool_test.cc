...
TEST(DummyLitePoolTest, SplitZeroAction) {
  ...
  dummy::DummyLitePool litepool(spec);
  ...
  DummyState state(litepool.Recv());
  ...
  litepool.Reset(action["env_id"_]);
  litepool.Recv();
  ...
  // send
  litepool.Send(action);
  state = DummyState(litepool.Recv());
  EXPECT_EQ(action["env_id"_].Shape(0), state["info:env_id"_].Shape(0));
  EXPECT_EQ(state["info:players.env_id"_].Shape(0), 8);
  for (int i = 0; i < 4; ++i) {
    EXPECT_EQ(static_cast<int>(state["info:env_id"_][i]), i);
...
TEST(DummyLitePoolTest, SinglePlayer) {
...
TEST(DummyLitePoolTest, MultiPlayers) {
