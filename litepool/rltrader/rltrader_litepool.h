...
class RlTraderEnvFns {
 public:
   ...
   *
   * LitePool will append to your configuration some common fields, currently
   * there are the litepool specific configurations that defines the behavior of
   * litepool.
   *
   * 1. num_envs: number of envs to be launched in litepool
   * 2. batch_size: the batch_size when interacting with the litepool
   * 3. num_threads: the number of threads to run all the envs
   * 4. thread_affinity_offset: sets the thread affinity of the threads
   * 5. base_path: contains the path of the litepool python package
   * 6. seed: random seed
   *
   ...
   *
   * Similarly, litepool also append to this state spec, there're:
   *
...
/**
 * Create an RlTraderEnvSpec by passing the above functions to EnvSpec.
 */
 ...
 * RlTraderEnvSpec we defined above.
 */
class RlTraderEnv : public Env<RlTraderEnvSpec> {
 ...
 public:
   ...
   * call `Allocate` to create the state (which is managed by litepool), and
  ...
  void Reset() override {
    ...
    // Ask litepool to allocate a piece of memory where we can write the state
    // after reset.
  ...
  void Step(const Action& action) override {
    ...
    // Parse the action, and execute the env (rltrader env has nothing to do)
    ...
    // Check if actions can successfully pass into litepool
...
/**
 * Pass the RlTraderEnv we defined above as an template parameter to the
 * AsyncLitePool template, it gives us a parallelized version of the single env.
 */
